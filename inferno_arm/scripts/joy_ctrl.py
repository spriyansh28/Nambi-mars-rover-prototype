...
class Arm:
	def __init__(self):
		...
		self.rate = rospy
		rospy.Subscriber("j1", Joy, self.joyCallback)
		self.set = Pwm()
 ...
 """
	...
	def joyCallback(self, msg):
		...
		if(abs(msg.axes[4]) > 0.2):
			self.set.pitch = int(255.0*msg.axes[4])
		...
		self.set.gripper = 255*(msg.buttons[0]-msg.buttons[3])

		self.set.roll = -255*(msg.buttons[1]-msg.buttons[2])
